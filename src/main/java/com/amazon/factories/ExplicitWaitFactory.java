...
public final class ExplicitWaitFactory {
	 ...
	 * {@link WaitStrategy}
	 * <p>
	 * In case of any issue with finding web elements using wait, then
	 * {@link WebElementNotFoundException} exception will be thrown
	 * 
	...
	public static WebElement performExplicitWait(WaitStrategy waitStrategy, By by) {
		...
		try {
			ExpectedCondition<WebElement> expCondition = null;
			
			if (waitStrategy == WaitStrategy.CLICKABLE) {
				expCondition = ExpectedConditions.elementToBeClickable(by);
			} else if (waitStrategy == WaitStrategy.PRESENCE) {
				expCondition = ExpectedConditions.presenceOfElementLocated(by);
			} else if (waitStrategy == WaitStrategy.VISIBLE) {
				expCondition = ExpectedConditions.visibilityOfElementLocated(by);
			}
			element = new WebDriverWait(DriverManager.getDriver(),
					Duration.ofSeconds(FrameworkConstants.getEXPLICITWAITTIME())).until(expCondition);
		} catch (Exception e) {
